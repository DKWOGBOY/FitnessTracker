...
export default function CaloriesChart({ days, targets }: { days: DayMacros[]; targets: Target[] }) {
  ...
  const data = days.map((d) => {
    const target = targetForDate(d.date);
    return {
      date: d.date,
      calories: Number(formatEnergy(d.calories, energyUnit)),
      target: target !== null ? Number(formatEnergy(target, energyUnit)) : null,
    };
  });
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <ComposedChart data={data} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
        ...
        />
        <YAxis tick={{ fontSize: 11, fill: "var(--color-text-muted)" }} unit={` ${energyUnitLabel(energyUnit)}`} />
